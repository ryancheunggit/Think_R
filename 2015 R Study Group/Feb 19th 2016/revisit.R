...
aaa <- as.data.frame(predict(dummyVars(~Pclass, dfull, levelsOnly = T), dfull))

d <- cbind.data.frame(
    as.data.frame(predict(dummyVars(~Pclass, dfull, levelsOnly = T), dfull)),
    as.data.frame(predict(dummyVars(~Sex, dfull, levelsOnly = T), dfull)),
    Age = dfull$Age,
    SibSp = dfull$SibSp,
    Parch = dfull$Parch,
    Fare = dfull$Fare,
    as.data.frame(predict(dummyVars(~Embarked, dfull, levelsOnly = T), dfull)),
    as.data.frame(predict(dummyVars(~Title, dfull, levelsOnly = T), dfull)),
...
bst <- xgboost(data = dtrainM, objective = "binary:logistic", metrics = "error", nrounds = m, verbose =  1)
